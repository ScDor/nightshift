...
func TestBuildPlanPrompt_CommitNormalizeGuardrails(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Scope: normalize only commits you create for this task or commits that exist only on your feature branch and are not yet shared.",
		"Safety: do not rewrite the primary branch, protected branches, tags, or already-shared history; explain the limitation instead.",
	} {
...
func TestBuildPlanPrompt_NonCommitNormalizeOmitsRewriteGuardrails(t *testing.T) {
	...
	prompt := o.buildPlanPrompt(task)
	if strings.Contains(prompt, "already-shared history") {
		t.Errorf("plan prompt should not include commit-normalize rewrite guardrails\nGot:\n%s", prompt)
	}
}
